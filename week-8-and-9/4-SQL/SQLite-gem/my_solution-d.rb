...
=begin
Finally, connecting Ruby to something! So, we can connect to a database now
all we need to do is connect to HTML and JavaScript and we can actually start
creating applications! Anyway, this was a fun challenge that combined SQL and
Ruby which is just awesome.

Here are my answers to the questions above:
Q: How does the sqlite3 gem work?
	A: I have no idea 'how' it works, but I can vaguely say what it 'does'. It
	allows a Ruby script to connect to an SQLite database and to run queries.
Q: What is the variable `$db` holding?
	A: It holds a pointer to the database connection established by the preceding
	statement of course; whatever that is...
Q: How does the `#execute` method work?
	A: It sends an SQL statement to a DBMS to be executed and returns the resulting
	table. As for 'how' it works I haven't the faintest idea.
Q: Explain the line `$db.execute("SELECT name FROM congress_members WHERE 
years_in_congress > #{minimum_years}")`
	A: Basically, this statement sends the 'SELECT ...' query to the database connection
	stored in the `$db` variable to be executed by the DBMS and have it's results
	returned.
=end